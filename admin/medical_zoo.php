<?php
    ...
    ?>
        <tr>
            ...
            <?php
                $stid = $row["id"];
                $zoo = "SELECT e.obt_mks,e.total_mks,e.status FROM `zoology` e WHERE e.student_id = $stid ORDER BY e.id DESC";
                $zoo_res = mysqli_query($db, $zoo);
                $zoo_row = mysqli_fetch_array($zoo_res);
            ?>
            <td><input style="background: transparent; border: none;" class="text-light text-center" type="text" name="obt<?php echo $i; ?>" value="<?php echo $zoo_row["obt_mks"] . "/" . $zoo_row["total_mks"]; ?>" readonly></td>
            <td><input style="background: transparent; border: none;" class="text-light" type="text" name="obt<?php echo $i; ?>" value="<?php echo $zoo_row["status"]; ?>" readonly></td>
