<?php
    ...
    ?> 
        <tr>
            ...
            <?php
                $stid = $row["id"];
                $mth = "SELECT e.obt_mks,e.total_mks,e.status FROM `math` e WHERE e.student_id = $stid ORDER BY e.id DESC";
                $mth_res = mysqli_query($db, $mth);
                $mth_row = mysqli_fetch_array($mth_res);
            ?>
            <td><input style="background: transparent; border: none;" class="text-light text-center" type="text" name="obt<?php echo $i; ?>" value="<?php echo $mth_row["obt_mks"] . "/" . $mth_row["total_mks"]; ?>" readonly></td>
            <td><input style="background: transparent; border: none;" class="text-light" type="text" name="obt<?php echo $i; ?>" value="<?php echo $mth_row["status"]; ?>" readonly></td>
