<?php
    ...
    ?> 
        <tr>
            ...
            <?php
                $stid = $row["id"];
                $chem = "SELECT e.obt_mks,e.total_mks,e.status FROM `chemistry` e WHERE e.student_id = $stid ORDER BY e.id DESC";
                $chem_res = mysqli_query($db, $chem);
                $chem_row = mysqli_fetch_array($chem_res);
            ?>
            <td><input style="background: transparent; border: none;" class="text-light text-center" type="text" name="obt<?php echo $i; ?>" value="<?php echo $chem_row["obt_mks"] . "/" . $chem_row["total_mks"]; ?>" readonly></td>
            <td><input style="background: transparent; border: none;" class="text-light" type="text" name="obt<?php echo $i; ?>" value="<?php echo $chem_row["status"]; ?>" readonly></td>
