<?php
    ...
    ?>
        <tr>
            ...
            <?php
                $stid = $row["id"];
                $eco = "SELECT e.obt_mks,e.total_mks,e.status FROM `economics` e WHERE e.student_id = $stid ORDER BY e.id DESC";
                $eco_res = mysqli_query($db, $eco);
                $eco_row = mysqli_fetch_array($eco_res);
            ?>
            <td><input style="background: transparent; border: none;" class="text-light text-center" type="text" name="obt<?php echo $i; ?>" value="<?php echo $eco_row["obt_mks"] . "/" . $eco_row["total_mks"]; ?>" readonly></td>
            <td><input style="background: transparent; border: none;" class="text-light" type="text" name="obt<?php echo $i; ?>" value="<?php echo $eco_row["status"]; ?>" readonly></td>
