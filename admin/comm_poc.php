<?php
    ...
    ?>
        <tr>
            ...
            <?php
                $stid = $row["id"];
                $poc = "SELECT e.obt_mks,e.total_mks,e.status FROM `poc` e WHERE e.student_id = $stid ORDER BY e.id DESC";
                $poc_res = mysqli_query($db, $poc);
                $poc_row = mysqli_fetch_array($poc_res);
            ?>
            <td><input style="background: transparent; border: none;" class="text-light text-center" type="text" name="obt<?php echo $i; ?>" value="<?php echo $poc_row["obt_mks"] . "/" . $poc_row["total_mks"]; ?>" readonly></td>
            <td><input style="background: transparent; border: none;" class="text-light" type="text" name="obt<?php echo $i; ?>" value="<?php echo $poc_row["status"]; ?>" readonly></td>
