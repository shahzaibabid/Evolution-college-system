<?php
    ...
    ?>
        <tr>
            ...
            <?php
                $stid = $row["id"];
                $bot = "SELECT e.obt_mks,e.total_mks,e.status FROM `botany` e WHERE e.student_id = $stid ORDER BY e.id DESC";
                $bot_res = mysqli_query($db, $bot);
                $bot_row = mysqli_fetch_array($bot_res);
            ?>
            <td><input style="background: transparent; border: none;" class="text-light text-center" type="text" name="obt<?php echo $i; ?>" value="<?php echo $bot_row["obt_mks"] . "/" . $bot_row["total_mks"]; ?>" readonly></td>
            <td><input style="background: transparent; border: none;" class="text-light" type="text" name="obt<?php echo $i; ?>" value="<?php echo $bot_row["status"]; ?>" readonly></td>
