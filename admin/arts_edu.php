<?php
    ...
    ?>
        <tr>
            ...
            <?php
                $stid = $row["id"];
                $edu = "SELECT e.obt_mks,e.total_mks,e.status FROM `education` e WHERE e.student_id = $stid ORDER BY e.id DESC";
                $edu_res = mysqli_query($db, $edu);
                $edu_row = mysqli_fetch_array($edu_res);
            ?>
            <td><input style="background: transparent; border: none;" class="text-light text-center" type="text" name="obt<?php echo $i; ?>" value="<?php echo $edu_row["obt_mks"] . "/" . $edu_row["total_mks"]; ?>" readonly></td>
            <td><input style="background: transparent; border: none;" class="text-light" type="text" name="obt<?php echo $i; ?>" value="<?php echo $edu_row["status"]; ?>" readonly></td>
